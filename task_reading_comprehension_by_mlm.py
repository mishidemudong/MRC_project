...
config['best_weights_name'] = './best_model.weights'

params_file = os.path.join('./','config.json')
...
if __name__ == '__main__':
    ...
    if False:
        ...
        model.load_weights('./best_model.weights') #best_model_0807_bert
        
        
        test = {"answer": "延安市", 
         "passage": "在职干警中,党员共38人 同义词 吴起县人民法院一般指延安市吴起县人民法院 吴起县人民法院位于吴起县城胜利大街中街。...吴起法院近三年来先后被省高级人民法院评为“全省法院司法宣传调研工作先进单位”、“全省法院网络宣传工作先进集体”、“政法干警核心价值观宣传工作先进单位”、“档案先进工作单位”和“司法宣传工作先进集体”。..."
            , 
        "question": "吴起县属于哪个市", 
            "id": "10013"
        }
        a_result = gen_answer(test['question'], [test['passage']])
        final_result = max_in_dict(a_result)
